...
"대용량 파일의 경우 일부분만 크롭해서 더빙하고 싶다"는 요구사항을 즉시 반영했습니다.
무거운 비디오 파일을 클라이언트나 서버에서 억지로 자르지 않고, **STT(음성 인식)에서 반환된 단어별 타임스탬프 결과 배열을 `startTime`과 `endTime` 범위 내로 필터링(`Array.filter`)**하는 논리적 크롭 방식을 고안하여 백엔드 파이프라인에 주입했습니다. 이로써 불필요한 번역/TTS 토큰 지출을 막으면서도 즉각적인 부분 더빙이 가능해졌습니다.

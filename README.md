...
## 📌 서비스 소개 및 주요 기능
Next.js(App Router) 기반으로 구축된 다국어 AI 더빙 파이프라인 웹 서비스입니다. 
사용자의 미디어 파일(오디오 및 비디오)을 업로드받아 **음성 추출 → 전사(STT) → 번역 → 음성 합성(TTS) → 비디오 병합** 단계를 원클릭으로 제공합니다.

### ✨ 핵심 기능 요약
- 🎞️ **미디어 다국어 더빙**: 비디오(MP4 등) 업로드 시 원본 영상 프레임 트랙에 새로운 다국어 고품질 음성(ElevenLabs)을 덧입혀 합성합니다.
- ✂️ **구간 선택 더빙 (논리적 크롭)**: 대용량 영상 전체를 처리하지 않고, 원하는 시작/종료 시간(초) 내의 대사 구간만 타겟팅하여 비용과 시간을 최적화합니다.
- 📸 **클라이언트 사이드 영상 렌더링**: Canvas와 MediaRecorder 웹 API를 활용해, 무거운 서버 인프라 의존 없이 브라우저 내에서 즉각적으로 합성 영상을 WebM으로 다운로드합니다. 
- 🔒 **화이트리스트 접근 제어**: 지정된 허용 이메일(kts123@estsoft.com 등) 사용자만 대시보드에 접근할 수 있도록 Turso DB와 결합된 NextAuth 보안 미들웨어가 강력히 보호합니다.
...
### 2. 구간 선택 더빙 (비용 및 시간 최적화)
"대용량 파일의 경우 전체가 아닌 대사 일부분만 타겟팅해서 더빙하고 싶다"는 요구사항을 즉시 반영했습니다.
무거운 비디오 파일을 클라이언트나 서버에서 억지로 자르지 않고, **STT(음성 인식)에서 반환된 단어별 타임스탬프 결과 배열을 `startTime`과 `endTime` 범위 내로만 필터링(`Array.filter`)**하는 논리적 크롭 방식을 고안하여 백엔드 파이프라인에 주입했습니다. 이로써 불필요한 번역/TTS 토큰 지출을 막으면서 즉각적인 부분 더빙이 가능해졌습니다.

### 3. 더빙 영상 클라이언트 스탠드얼론 병합(Muxing) 및 다운로드

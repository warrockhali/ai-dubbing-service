...
### 8. Vercel 4.5MB 업로드 제한 극복: 아키텍처 한계 내에서의 극한 최적화 (Web Audio API Fallback)
평가단 환경(기업 관리형 브라우저 등)에서 WebAssembly(WASM) 실행이 보안 정책으로 원천 차단되어 크롭 로직이 붕괴하고 4.5MB 한계 에러(`413 Payload Too Large`)가 발생하는 크리티컬한 엣지 케이스가 있었습니다. 
일반적인 해결책이라면 AWS S3 Presigned URL이나 Vercel Blob 등 외부 스토리지를 연동하여 아키텍처를 우회했겠지만, **"무료 플랜과 선언한 기존 아키텍처 내에서 끝까지 해결을 도출하자"**는 제약적 한계 돌파를 목표로 에이전트에게 **"브라우저의 순수 네이티브 API만을 사용하여 용량을 극도로 압축해라"**고 지시했습니다.

에이전트와의 협력 끝에, WASM이 없더라도 실행 가능한 **`OfflineAudioContext`(Web Audio API)를 활용하여 원본 대용량 비디오에서 20초 분량의 오디오 트랙만 분리한 뒤, 이를 16,000Hz Mono 채널 파형(WAV)으로 브라우저 단에서 강제 다운샘플링**하는 Fallback 로직을 완성했습니다. 
그 결과, 수십~수백 MB의 영상 파일이라도 Vercel 서버로 전송되는 페이로드는 1MB 이하의 초경량 오디오 파일로 변환되어 전송되므로 **4.5MB 인프라 제한을 완벽하게(100%) 피해 갈 수 있게** 되었습니다. 외부 클라우드 의존성 없이 주어진 브라우저 자원만으로 서버의 제약을 극복한 가장 성공적인 트러블슈팅 사례입니다.
